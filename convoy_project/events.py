...
class Events(object):
    ...
    def event_generator(self,game_time):
        
        dictionary_events = {'convoy_lost': 0, 'raiders': 0, 'sand_storm': 0}
        
        convoy_lost_probability = 0
        raiders_probability = 0
        sand_storm_probability = 0
        ...
        event_happens = random.randint(0,5) 
        if event_happens >= 0 and event_happens <= 2:#2 in 5 possibilities  for any event to happen.
            ...
            maxEvent = max(dictionary_events, key=dictionary_events.get) #this function gets the element of the dictionary with the higher value.
            return maxEvent